...
class GatedCNNModel(Trainer) :
    def build_model(self, nb_filter=100, filter_lens=range(1,6), reg=0.00001):
        ...
        for aspect in self.aspects + ['E']:
            network1 = sum_normalize(gated_cnn(lookup, 1, 200, reg))
            network2 = sum_normalize(gated_cnn(lookup, 2, 200, reg))
            network3 = sum_normalize(gated_cnn(lookup, 3, 200, reg))
            network4 = sum_normalize(gated_cnn(lookup, 4, 200, reg))
            network = Concatenate()([network1, network2, network3, network4])
