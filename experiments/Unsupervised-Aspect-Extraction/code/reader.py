...
def create_vocab(domain, maxlen=0, vocab_size=0):
    ...
    vocab_file = codecs.open('../preprocessed_data/%s/vocab' % domain, encoding='utf8')
    vocab = {}
    if vocab_file :
        for i, line in enumerate(vocab_file.readlines()) :
            line = line.split('\t')
            vocab[line[0]] = i
        return vocab
...
def read_dataset(domain, phase, vocab, maxlen):
    ...
    output = '../preprocessed_data/'+domain+'/'+phase+'.p'
    ...
    pickle.dump((data_x, maxlen_x), open(output, 'wb'))
