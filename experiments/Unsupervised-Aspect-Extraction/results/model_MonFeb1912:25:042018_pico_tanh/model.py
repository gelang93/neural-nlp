...
class EmbedSentence(nn.Module) :
    ...
    def forward(self, sentence, imask, reconstruct=False, do_tanh=False) :
        #(B, L), #(B, L)
        ...
        if do_tanh :
            attn2 = -1.0*(F.tanhshrink(attn) - attn)
        else :
            attn2 = attn
        attn3 = attn2.masked_fill_(imask.byte().unsqueeze(-1), -float('inf')) # (B, L, 1)
        attn_weights = F.softmax(attn3, dim=1) # (B, L, 1)
...
class Model() :
    ...
    def train_batch(self, input_batch, negative_batch, update=True, do_tanh=False) :
        model = self.model
        ...
        mix_s, attn_weights, p_t, r_s = model(si, smask, reconstruct=True, do_tanh=do_tanh) #(B, E), _, (B, K), (B, E)
        ...
        # self.p_t = p_t.data
        # self.r_s = r_s.data
        # self.attn_weights = attn_weights.data

        for n in range(len(negative_batch)) :
            ni, nmask = self.prepare_data(negative_batch[n])
            mix_n, _,_,_ = model(ni, nmask, do_tanh=do_tanh)
