...
class OneCNNModel(Trainer) :
    def build_model(self, nb_filter=100, filter_lens=range(1,6), reg=0.000):
        ...
        cnn_network, convs = cnn_embed(lookup, filter_lens, nb_filter*4, maxlen, word_dim, reg, name='pool')
        weighted = Element_wise_weighting(nb_aspects=len(self.aspects))(cnn_network)

        
        models = OrderedDict()
        for aspect in self.aspects:
            network = Lambda(lambda s : s[:, :, int(aspect)])(weighted)
