...
class GCNN(Layer):
    ...
    def build(self, input_shape):
        ...
        self.kernel_shape = (self.window_size, 1, input_dim, self.output_dim * 3)
        ...
        if self.use_bias:
            self.bias = self.add_weight((self.output_dim * 3,),
    ...
    def compute_output_shape(self, input_shape):
        ...
        if length:
            length = conv_output_length(length + self.window_size - 1,
                                        ...
                                        self.strides[0])
        return (input_shape[0], length, self.output_dim, 2)
    ...
    def call(self, x):
        ...
        g = output[:, :, self.output_dim:(self.output_dim*2)]
        h = output[:, :, (self.output_dim*2): ]
        concat = K.stack([g, h], axis=-1)
        concat = K.softmax(concat)
        z = self.activation(z)
        z = K.expand_dims(z, axis=-1)
        return self.activation(z) * concat
