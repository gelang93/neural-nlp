...
class NVDMModel(Trainer) :
    def build_model(self, nb_filter=100, filter_lens=range(1,6), reg=0.000001):
        ...
        for aspect in self.aspects :
            ...
            diff1 = Dot(axes=-1, normalize='l2')([embed_1[0], embed_2[0]])
            diff2 = Dot(axes=-1, normalize='l2')([embed_1[0], embed_3[0]])

            vaeterms = Add()([embed_1[1], embed_2[1], embed_3[1]])
            
            model_pred = Model([input_1, input_2, input_3], [diff1, diff2, vaeterms])
            model_pred.name = 'pred_'+aspect
            models_pred[aspect] = model_pred

        P = OrderedDict()
        for aspect in self.aspects :
            P[aspect] = models_pred[aspect]([I['O'], I[(aspect, 'S')], I[(aspect, 'D')]])
        ...
        for aspect in self.aspects :
            ...
            self.losses[name_v] = lambda y_true, y_pred : 0.01 * K.mean(y_pred)
